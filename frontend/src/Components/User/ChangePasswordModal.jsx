...
function ChangePasswordModal({setChange}) {
    ...
    return createPortal(
        <div className='fixed h-screen w-screen z-20 top-0 bg-[#0000007e] flex justify-center items-center'>
            <div className='bg-white dark:bg-black dark:text-white rounded-md w-[80%] sm:w-[70%] md:w-[55%] lg:w-[40%] xl:w-[30%] p-8 relative flex flex-col gap-5'>
                <div className='text-black dark:text-white cursor-pointer absolute right-5 top-5' onClick={()=>{setChange(false);}}>
                ...
                <button onClick={handleChangePassword} className="bg-black dark:bg-white text-white dark:text-black py-2 rounded-sm w-full tracking-wider font-semibold">CHANGE PASSWORD</button>
            </div>
        </div>,
